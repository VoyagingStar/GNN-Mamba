...
# =========================
INPUT_CSV = Path(r"E:\model\singlecell\results\HGC\0330\HMDB_DecoID\library_weights_all_queries.csv")
OUTPUT_DIR = Path(r"E:\model\singlecell\results\HGC\0330\HMDB_DecoID\data")
...
def load_data(csv_path: Path) -> pd.DataFrame:
    ...
    df["hmdb_precursor_mz"] = pd.to_numeric(df["hmdb_precursor_mz"], errors="coerce")
    df["lasso_weight"] = pd.to_numeric(df["lasso_weight"], errors="coerce")
    df["lasso_weight_norm"] = pd.to_numeric(df["lasso_weight_norm"], errors="coerce")
    ...
    # Normalize identifiers for deduplication and counting
    ...
    df.loc[df["hmdb_id"].isin(["", "nan", "None", "none", "NaN"]), "hmdb_id"] = pd.NA
...
def build_count_summary(df: pd.DataFrame, thresholds: list[int]) -> pd.DataFrame:
    ...
    for threshold in thresholds:
        ...
        # Ensure all query_csv are included, even if count is 0
        ...
        # Save the per-query detail as a companion file
...
def choose_representative_values(group: pd.DataFrame) -> pd.Series:
    ...
    Note:
    - group is already filtered by filter_annotated(), so component_score statistics
      are calculated only on the filtered rows.
    - best_component_score / mean_component_score are both derived directly from
      library_weights_all_queries.csv, no longer from decoid_component_ranking_all_queries.csv.
    """
    g = group.copy()
    ...
    g = g.sort_values(
        by=["lasso_weight_norm", "initial_candidate_score", "initial_candidate_n_matched"],
    ...
    return pd.Series(
        {
            ...
            "hmdb_precursor_mz": row.get("hmdb_precursor_mz"),
            ...
            "best_lasso_weight_raw": g["lasso_weight"].max(),
            "best_lasso_weight_normalized": g["lasso_weight_norm"].max(),
...
def build_occurrence_table(df: pd.DataFrame, min_matched: int) -> pd.DataFrame:
    ...
    metadata = filtered.groupby("inchikey", dropna=False).apply(
        choose_representative_values,
