...
Use MoNA library spectra to deconvolve unknown spectra with DecoID-style LASSO/NNLS,
while keeping the overall workflow consistent with HMDB_DecoID.py.

Design principles:
- Keep the deconvolution / scoring / output logic aligned with HMDB_DecoID.py
- Replace HMDB-specific library loading with MoNA-style JSON/JSONL parsing
- Preserve useful MoNA metadata in outputs:
  inchikey, smiles, formula, adduct, ion_mode, instrument, collision_energy, source_file
- No CLI; edit CONFIG directly
"""
...
MONA_LIB_PATH = r"E:\model\singlecell\spectra_library\MoNA-export-LC-MS-MS_Positive_Mode.json"
MONA_ION_MODE_KEEP: Optional[str] = "positive"

OUTPUT_DIR = r"E:\model\singlecell\results\HGC\0330\MoNA_DecoID"
...
def _compound0(entry: Dict[str, Any]) -> Optional[Dict[str, Any]]:
...
def _find_anywhere(entry: Dict[str, Any], keys: List[str]) -> Optional[str]:
    for k in keys:
...
def parse_mona_spectrum_string(spectrum: str) -> List[Tuple[float, float]]:
...
def parse_mona_peaks(entry: Dict[str, Any]) -> List[Tuple[float, float]]:
    spec = entry.get("spectrum")
    if isinstance(spec, str) and spec.strip():
        return parse_mona_spectrum_string(spec)
    ...
    peaks_raw = entry.get("PEAKS")
    if peaks_raw is not None:
        return _parse_peaks_literal(peaks_raw)

    return []


def extract_precursor_mz(entry: Dict[str, Any]) -> Optional[float]:
    for k in ("Precursor_MZ", "precursor_mz", "precursorMz", "PEPMASS", "pepmass", "PRECURSOR_MZ"):
        if k in entry:
            mz = _safe_float(entry.get(k))
            if mz is not None:
                return mz

    v = _find_metadata_value(
        entry.get("metaData") or entry.get("metadata"),
    ...
    c0 = _compound0(entry)
    if c0:
        v2 = _find_metadata_value(
            c0.get("metaData") or c0.get("metadata"),
            {"precursor m/z", "precursormz", "pepmass", "precursor"},
        )
        mz2 = _safe_float(v2) if v2 is not None else None
        if mz2 is not None:
            return mz2
...
def extract_name(entry: Dict[str, Any]) -> str:
    v = _find_anywhere(entry, ["compound_name", "Compund_Name", "NAME", "Name", "name", "TITLE", "title"])
    if v:
        return v
...
def extract_inchikey(entry: Dict[str, Any]) -> str:
    v = _find_anywhere(entry, ["InChIKey", "inchikey", "inchi_key", "inchiKey"])
    if v:
        return v
    v1 = _find_metadata_value(entry.get("metaData") or entry.get("metadata"), {"inchikey", "inchi key"})
    if v1:
        return v1.strip()
    c0 = _compound0(entry)
    if c0:
        v2 = _find_metadata_value(c0.get("metaData") or c0.get("metadata"), {"inchikey", "inchi key"})
        if v2:
            return v2.strip()
    return ""
...
def extract_smiles(entry: Dict[str, Any]) -> str:
    v = _find_anywhere(entry, ["SMILES", "Smiles", "smiles"])
    if v:
        return v
    v1 = _find_metadata_value(entry.get("metaData") or entry.get("metadata"), {"smiles"})
    if v1:
        return v1.strip()
    c0 = _compound0(entry)
    if c0:
        v2 = _find_metadata_value(c0.get("metaData") or c0.get("metadata"), {"smiles"})
        if v2:
            return v2.strip()
    return ""
...
def extract_formula(entry: Dict[str, Any]) -> str:
    v = _find_anywhere(entry, ["formula", "Formula", "molecularFormula", "molecular_formula"])
    if v:
        return v
    v1 = _find_metadata_value(entry.get("metaData") or entry.get("metadata"), {"molecular formula", "formula"})
    if v1:
        return v1.strip()
    c0 = _compound0(entry)
    if c0:
        v2 = _find_metadata_value(c0.get("metaData") or c0.get("metadata"), {"molecular formula", "formula"})
        if v2:
            return v2.strip()
    return ""
...
def extract_adduct(entry: Dict[str, Any]) -> str:
    v1 = _find_metadata_value(entry.get("metaData") or entry.get("metadata"), {"precursor type", "adduct"})
    if v1:
        return v1.strip()
    c0 = _compound0(entry)
    if c0:
        v2 = _find_metadata_value(c0.get("metaData") or c0.get("metadata"), {"precursor type", "adduct"})
        if v2:
            return v2.strip()
    return ""
...
def extract_ion_mode(entry: Dict[str, Any]) -> str:
    v1 = _find_metadata_value(
        entry.get("metaData") or entry.get("metadata"),
        {"ionization mode", "ion mode", "ionmode", "polarity"},
    )
    if v1:
        return v1.strip()
    c0 = _compound0(entry)
...
def extract_instrument(entry: Dict[str, Any]) -> str:
    v1 = _find_metadata_value(entry.get("metaData") or entry.get("metadata"), {"instrument", "instrument type"})
    if v1:
        return v1.strip()
    c0 = _compound0(entry)
    if c0:
        v2 = _find_metadata_value(c0.get("metaData") or c0.get("metadata"), {"instrument", "instrument type"})
        if v2:
            return v2.strip()
    return ""
...
def extract_collision_energy(entry: Dict[str, Any]) -> str:
    v1 = _find_metadata_value(
        entry.get("metaData") or entry.get("metadata"),
        {"collision energy", "collisionenergy", "ce", "nce"},
    )
    if v1:
        return v1.strip()
    c0 = _compound0(entry)
...
def iter_json_entries_stream(path: Union[str, Path]) -> Iterator[Tuple[Dict[str, Any], str]]:
    p = Path(path)
    if not p.exists():
        raise FileNotFoundError(f"MoNA JSON not found: {p}")
    ...
    if ch == "[":
        try:
            ...
            raise RuntimeError(
                f"{p.name} seems to be a large JSON array. Install ijson for streaming:\n"
                f"  pip install ijson\n"
                f"Import error: {e}"
            )
        with p.open("rb") as f:
...
def load_mona_spectra(mona_path: Union[str, Path], ion_mode_keep: Optional[str] = None) -> List[Dict[str, Any]]:
    mona_path = Path(mona_path)
    print(f"Loading MoNA library: {mona_path}")

    spectra: List[Dict[str, Any]] = []
    counter = 0
    for entry, src in iter_json_entries_stream(mona_path):
        if not isinstance(entry, dict):
            continue

        ion_mode = extract_ion_mode(entry)
        if ion_mode_keep is not None:
            if ion_mode and ion_mode.strip().lower() != str(ion_mode_keep).strip().lower():
                continue

        peaks = clean_and_normalize_peaks(
            parse_mona_peaks(entry),
        ...
        precursor_mz = extract_precursor_mz(entry)
        if precursor_mz is None or precursor_mz <= 0:
            continue

        counter += 1
        spectrum_id = str(entry.get("spectrum_id") or entry.get("id") or entry.get("ID") or f"MoNA_{counter}")
        ...
        spectra.append(
            {
                ...
                "best_adduct": "" if adduct is None else str(adduct),
                "ion_mode": "" if ion_mode is None else str(ion_mode),
                "instrument": "" if instrument is None else str(instrument),
                "collision_energy": "" if collision_energy is None else str(collision_energy),
                "source_file": str(src),
            }
        )

    print(f"Parsed MoNA spectra: {len(spectra)}")
    return spectra
    ...
    for prec, sub in df.groupby("precursor_mz", dropna=False):
        ...
        peaks = list(
            zip(
                ...
                pd.to_numeric(sub[intensity_col_in], errors="coerce").fillna(0).astype(float).tolist(),
            )
        )
        peaks = clean_and_normalize_peaks(
            peaks,
...
def getMatricesForGroup(trees, spectra, mz_merge_tol, top_n_candidates, min_initial_cosine):
    ...
    return (
        mona_ids, spectraIDs, matrix, masses, compound_names, best_adducts, inchikeys,
        smiles_list, formulas, ion_modes, instruments, collision_energies, source_files,
        axis, reduceSpec, initial_scores, initial_matches
    )
...
def score_components_against_mona(
    component_vectors,
    mona_ids,
    spectraIDs,
    masses,
    ...
    for comp_info in component_vectors:
        ...
        component_hits = []
        for mona_id, spec_id, mass, name, best_adduct, inchikey, smiles, formula, ion_mode, instrument, ce, source_file, db_vec, init_score, init_n_match in zip(
            mona_ids, spectraIDs, masses, compound_names, best_adducts, inchikeys, smiles_list, formulas,
            ion_modes, instruments, collision_energies, source_files, comp_info["db_matrix"], initial_scores, initial_matches
        ):
...
def build_library_weight_rows(
    ...
    for idx, (mona_id, spec_id, mass, name, adduct, inchikey, smiles, formula, ion_mode, instrument, ce, source_file, init_score, init_n_match) in enumerate(
        zip(
            mona_ids, spectraIDs, masses, compound_names, best_adducts, inchikeys, smiles_list, formulas,
            ion_modes, instruments, collision_energies, source_files, initial_scores, initial_matches
        )
    ):
...
def attach_component_scores_to_library_weights(weight_rows, ranking_rows):
    ...
    for row in ranking_rows or []:
        ...
        mona_id = str(row.get("mona_id", ""))
        if component_name and mona_id and component_name == mona_id:
            score = float(row.get("component_score", 0.0))
            if mona_id not in score_map or score > score_map[mona_id]:
                score_map[mona_id] = score

    for row in weight_rows:
        mona_id = str(row.get("mona_id", ""))
        row["component_score"] = float(score_map.get(mona_id, 0.0))

    return weight_rows



def build_component_vectors(originalSpectra, matrix, res, mona_ids, masses, best_adducts, spectraIDs, compound_names, inchikeys, smiles_list, formulas):
    component_rows = []
...
class MonaLibraryAdapter:
    def __init__(self, mona_spectra: List[Dict[str, Any]]):
        self.mona_spectra = mona_spectra
    ...
    def get_candidate_trees(self, lower_bound: float, upper_bound: float):
        ...
        for rec in self.mona_spectra:
            ...
            if lower_bound <= mz <= upper_bound:
                ...
                spec_id = str(rec.get("spectrum_id", rec.get("mona_id", "MoNA")))
...
def deconvolve_and_score_one_query(
    ...
    rows = score_components_against_mona(
    ...
    df_weights["n_candidates"] = int(len(mona_ids))
    ...
    df["n_candidates"] = int(len(mona_ids))
    df["s2n"] = float(s2n)
...
def process_one_query_csv(query_csv: Path, mona_spectra: List[Dict[str, Any]], out_root: Path) -> Dict[str, pd.DataFrame]:
    ...
    library = MonaLibraryAdapter(mona_spectra)
    all_parts = []
    all_weight_parts = []
...
def main():
    ...
    mona_spectra = load_mona_spectra(MONA_LIB_PATH, ion_mode_keep=MONA_ION_MODE_KEEP)
    if not mona_spectra:
        raise RuntimeError("No MoNA spectra parsed. Check MONA_LIB_PATH / MONA_ION_MODE_KEEP.")

    all_dfs = []
    all_weight_dfs = []
    for qcsv in query_files:
        result_one = process_one_query_csv(qcsv, mona_spectra, out_root)
