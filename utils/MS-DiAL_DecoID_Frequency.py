# -*- coding: utf-8 -*-
"""
MSDIAL_DecoID_Frequency.py

参考：
- MoNA_DecoID_Frequency.py 的统计流程
- HMDB_DecoID_Frequency.py 的 occurrence_query_csv_count 输出方式
- MS-DiAL_DecoID.py 的 library_weights_all_queries.csv 列名与数据格式

功能：
1) 从 MS-DiAL_DecoID.py 输出的 decoid_library_weights_all_queries.csv 读取结果
...
# =========================
INPUT_CSV = Path(r"E:\model\singlecell\results\HGC\0330\MS-DiAL_DecoID\decoid_library_weights_all_queries.csv")
OUTPUT_DIR = Path(r"E:\model\singlecell\results\HGC\0330\MS-DiAL_DecoID\data")
...
def load_data(csv_path: Path) -> pd.DataFrame:
    ...
    for col in ["inchikey", "msdial_id", "spectrum_id", "compound_name", "best_adduct", "smiles", "formula",
                "ion_mode", "instrument", "collision_energy", "ontology", "source_file"]:
        df[col] = df[col].astype(str).str.strip()

    df.loc[df["inchikey"].isin(["", "nan", "None", "none", "NaN", "<NA>"]), "inchikey"] = pd.NA
    df.loc[df["msdial_id"].isin(["", "nan", "None", "none", "NaN", "<NA>"]), "msdial_id"] = pd.NA
...
def build_occurrence_table(df: pd.DataFrame, min_matched: int) -> pd.DataFrame:
    ...
    # 显式排除 grouping 列，避免 pandas 的 GroupBy.apply deprecation warning
